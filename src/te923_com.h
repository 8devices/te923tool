...
int decode_te923_data( unsigned char buf[], Te923DataSet_t *data ) {
	...
	//decode pressure
	if ( debug > 0 )
		printf( "[DEBUG] PRESS BUF[20]=%02x BUF[21]=%02x\n", buf[20], buf[21] );
	if (( buf[21] & 0xF0 ) == 0xF0 ) {
		data->press = 0;
		data->_press = -1;
	} else {
	...
	//decode windchill
	if ( debug > 0 )
		printf( "[DEBUG] WCHIL BUF[23]=%02x BUF[24]=%02x\n", buf[23], buf[24] );
	...
	//decode windgust
	if ( debug > 0 )
		printf( "[DEBUG] WGUST BUF[25]=%02x BUF[26]=%02x\n", buf[25], buf[26] );
	...
	//decode windspeed
	if ( debug > 0 )
		printf( "[DEBUG] WSPEE BUF[27]=%02x BUF[28]=%02x\n", buf[27], buf[28] );
	...
	if ( debug > 0 )
		printf( "[DEBUG] RAINC BUF[29]=%02x BUF[30]=%02x BUF[31]=%02x\n", buf[29] & 0xF0, buf[30], buf[31] );
...
int get_te923_lifedata( usb_dev_handle *devh , Te923DataSet_t *data ) {
	// get the databuffer from device
	int ret;
	int adr = 0x020001;
	unsigned char buf[BUFLEN];
	do
