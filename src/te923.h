...
typedef struct {
	...
	int RainCount;				// Raincounter of station as number up to 65535 * 0.7 mm/m²
	// status of sensors, names are _<sensor>; if status is <> 0, the value should be 0
	// 0  => value of sensor should ok
	// -1 => data is invalid
	// -2 => sensor is out of range
	// -3 => missing link
	// -4 => any other error
	char _t[6], _h[6];
	char _press, _uv, _wDir, _wSpeed, _wGust, _wChill, _RainCount, _storm, _forecast;
	int __src;				// source adress of dataset (needed for dump)
} Te923DataSet_t;
...
_Bool battery1 :
	1;			// battery in sensor 1
_Bool battery2 :
	1;			// battery in sensor 2
_Bool battery3 :
	1;			// battery in sensor 3
_Bool battery4 :
	1;			// battery in sensor 4
_Bool battery5 :
	1;			// battery in sensor 5
_Bool batteryUV :
	1;			// battery in UV sensor
_Bool batteryWind  :
	1;			// battery in wind sensor
_Bool batteryRain :
	1;			// battery in rain sensor
} Te923DevSet_t;
